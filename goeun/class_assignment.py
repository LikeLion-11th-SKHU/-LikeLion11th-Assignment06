class Student :
    ...
    while Class_name != '종료' : #종료와 같지 않으면 계속 반복하도록 반복문 사용
        ...
        subject={} #빈 딕셔너리 선언. 입력받은 과목들을 위해 사용
        subject ={key:value}
    ...
    def subject_info() :
        print("자세한 수강목록입니다.")
        print(f'과목명:{key[0]}/과목명의 길이:{len(value[0])}')
        print(f'과목명:{key[1]}/과목명의 길이:{len(value[1])}')
        print(f'과목명:{key[2]}/과목명의 길이:{len(value[2])}')
        print(f'과목명:{key[3]}/과목명의 길이:{len(value[3])}')
